...
set_property -dict { PACKAGE_PIN E3  IOSTANDARD LVCMOS33 }       [get_ports { clk_100MHz_i }]; #IO_L12P_T1_MRCC_35 Sch=gclk[100]
#create_clock -add -name clk_100MHz -period 10.00 -waveform {0 5} [get_ports { clk_100MHz_i }];

# Do not time async reset path
set_false_path -from [get_ports {arstn_i}];
# UART does not need to be timed as it is slooooooooooow
set_false_path -from [get_ports {uart_rx_i}];
set_false_path -to   [get_ports {uart_tx_o}];
set_false_path -to   [get_ports {rst_led_o}];

set_property -dict { PACKAGE_PIN A8  IOSTANDARD LVCMOS33 } [get_ports { arstn_i }]; #IO_L12N_T1_MRCC_16 Sch=sw[0]

set_property -dict { PACKAGE_PIN G13 IOSTANDARD LVCMOS33 } [get_ports { uart_rx_i }]; #IO_0_15 Sch=ja[1]
set_property -dict { PACKAGE_PIN B11 IOSTANDARD LVCMOS33 } [get_ports { uart_tx_o }]; #IO_L4P_T0_15 Sch=ja[2]

set_property -dict { PACKAGE_PIN H5  IOSTANDARD LVCMOS33 } [get_ports { rst_led_o }]; #IO_L24N_T3_35 Sch=led[4]
